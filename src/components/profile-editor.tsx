...
export function ProfileEditor({ userId, onSaved }: { userId: string; onSaved?: () => void }) {
  ...
  useEffect(() => {
    (async () => {
      const { data } = await supabase.from("profiles").select("id,display_name,username,avatar_url,background_url,bio,hobby_tags,theme_color,age,gender").eq("id", userId).single();
      if (data) {
  ...
  async function handleFile(kind: "avatar" | "bg", file?: File | null) {
    ...
    try {
      const url = await uploadUserMedia(userId, kind === "avatar" ? "avatars" : "backgrounds", file);
  ...
  if (!profile) return <div className="empty-panel"><p>読み込み中…</p></div>;
